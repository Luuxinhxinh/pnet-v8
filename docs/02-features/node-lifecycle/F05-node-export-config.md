...
2. **Xác định Loại Thiết bị**:
   - Nếu là Cisco IOL: Đọc trực tiếp phân vùng NVRAM nhị phân bằng tiện ích `/opt/unetlab/scripts/iou_export` để giải mã text file `startup-config`.
...
| :--- | :--- | :--- |
| `/opt/unetlab/html/includes/api_nodes.php` | `apiNodeExport()` | Tiếp nhận request xuất cấu hình |
| `/opt/unetlab/html/includes/functions.php` | `nodeExport()` | Điều phối giải mã NVRAM hoặc gọi script |
| `/opt/unetlab/scripts/iou_export` | C binary | Trích xuất cấu hình từ file NVRAM của IOL |
| `/opt/unetlab/html/includes/__lab.php` | `Lab::saveConfig()` | Ghi chuỗi cấu hình vào cây XML của lab |
