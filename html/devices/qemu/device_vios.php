...
class device_vios extends device_qemu
{
    ...
    public function prepare()
    {
       ...
       // boots WITHOUT the Cisco IOSv eval banners (banner exec/incoming/login) and skips
       // the initial-config dialog. Was gated on Initial_startup_config==1 (off by default);
       // now ON by default — opt out per node by setting Initial_startup_config to '0'.
       ...
       // boot. Gate on .configured only; IOSv only auto-loads the config disk when its NVRAM
      ...
      public function customFlag($flag)
    {
        ...
        // post-wipe); IOSv ignores minidisk_initial once its NVRAM startup-config exists.
        else if (!is_file($this->getRunningPath() . '/.configured') && $this->Initial_startup_config != '0' )
        {
            $diskcmd = '/opt/unetlab/scripts/createdosdisk_vios.sh ' . $this->getRunningPath();
            exec($diskcmd, $o, $rc);    
