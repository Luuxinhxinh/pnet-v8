...
/**
 * device_catcontrol.php — Cisco Catalyst SD-WAN Controller (vSmart) day-0 prestage.
 *
 * Identical cidata-seed mechanism as device_catmanager.php (see that file for the rationale);
 * only the base cloud-config differs (startup_configs/sdwan/controller_user-data.txt — vSmart
 * personality). Default-ON; opt out per node via Initial_startup_config '0'. Saved
 * startup-config (the Builder path) is delivered verbatim.
 *
 * @author pnetlab-noble sdwan-builder
 * @link https://www.pnetlab.com/
 */

class device_catcontrol extends device_qemu
{
    ...
    /**
     * Label the link-picker interfaces eth0..ethN to match the names vSmart
    ...
    public function prepare()
    {
        ...
        if (is_file($rp . '/startup-config') && !is_file($rp . '/.configured')) {
            ...
            $file_contents = file_get_contents('/opt/unetlab/startup_configs/sdwan/controller_user-data.txt');
