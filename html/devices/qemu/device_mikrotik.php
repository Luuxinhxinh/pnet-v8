    ...
    public function createEthernets($quantity)
    {
        ...
        if ($this->inject_as_first_nic == 1) {
            ...
            if ($this->console == 'winbox' || $this->console_2nd == 'winbox' ){   
                             $ethernets[$quantity] = new Interfc( $this, array('name' => 'winbox', 'type' => 'ethernet',), $quantity);
            }

            if ($this->console == 'http' || $this->console_2nd == 'http' ){   
                            $ethernets[$quantity + 1 ] = new Interfc( $this, array('name' => 'Mgmt http', 'type' => 'ethernet',), $quantity + 1 );
                            $first ++;
                    }
            if ($this->console == 'https' || $this->console_2nd == 'https' ){   
                            $ethernets[$quantity + 2 ] = new Interfc( $this, array('name' => 'Mgmt https', 'type' => 'ethernet',), $quantity + 2);
                            $first ++;
            }
            if ($this->console == 'ssh' || $this->console_2nd == 'ssh' ){   
                             $ethernets[$quantity + 3 ] = new Interfc( $this, array('name' => 'Mgmt ssh', 'type' => 'ethernet',), $quantity + 3);
                             $first ++;
            }
            /*FOR CUSTOM COSNOLES */
        }
