...
// b2 amputation follow-up: adjustZoom() removed. It re-applied the jsPlumb
// #lab-viewport scale transform via window.setZoom(..., lab_topology, ...) —
// both the viewport and the jsPlumb instance are gone (the flow island owns zoom
// and the lab_topology stub has no getContainer), so a live call would have
// thrown TypeError. It was unreachable: its only caller is the branch below,
// gated on contextMenuOpen, which is initialised false
// (functions/core/globals.js:5) and never assigned true anywhere in the tree.
//
// hideContextmenu() itself is KEPT — ebs/actions.js binds it to every document
// click and to window blur — but reduced to the flag reset that was the only
// statement outside that dead branch.
