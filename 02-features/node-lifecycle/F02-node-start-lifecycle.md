...
5. **Kích hoạt Wrapper Nhị phân**:
   - PHP API gọi dòng lệnh hệ thống: `sudo /opt/unetlab/scripts/unl_wrapper.php -a start -T <tenant_pod> -D <node_id>`.
...
| :--- | :--- | :--- |
| `/opt/unetlab/html/api.php` | `$app->post("/api/labs/session/nodes/(:action)")` | Định tuyến hành động `start` |
| `/opt/unetlab/html/includes/api_nodes.php` | `apiNodeStart()` | Kiểm tra trạng thái và điều phối khởi động |
| `/opt/unetlab/html/includes/functions.php` | `nodeStart()` | Sinh thư mục tmp, chuẩn bị đĩa và gọi unl_wrapper |
| `/opt/unetlab/scripts/unl_wrapper.php` | `startNode()` | Script CLI phân tích loại thiết bị và gọi wrapper C |
| `/opt/unetlab/wrappers/qemu_wrapper` | C main process | Tạo TAP, cấu hình qemu cmdline và khởi chạy QEMU |
