...
| :--- | :--- | :--- |
| `/opt/unetlab/scripts/mcp/pnetlab-mcp.py` | `MCPServer`, `register_tools()` | Máy chủ MCP Server Python (52KB) |
| `/opt/unetlab/scripts/mcp/ai_lab_agent.py` | `AILabAgent` | Bộ xử lý lập luận và phân tách lệnh của AI |
| `/opt/unetlab/html/mcp/api.php` | PHP API | Endpoint tiếp nhận truy vấn MCP từ Web |
| `/opt/unetlab/html/mcp/bridge.php` | PHP | Cầu nối xác thực cookie token với MCP |
