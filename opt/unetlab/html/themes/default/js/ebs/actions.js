...
$(window).resize(function () {
    ...
    // b2: #lab-viewport is gone (the Svelte Flow island owns the canvas), so this
    // resize handler is a no-op — but it still CALLED getZoomLab() to build the
    // args, which threw if a resize fired before the stub installed getZoomLab.
    // Bail when the legacy viewport is absent, and guard the zoom read regardless.
    if (!viewport.length) return;
    var _zoom = (typeof getZoomLab === 'function') ? getZoomLab() : 100;
    viewport.width(($(window).width()) * (100/_zoom));
    viewport.height($(window).height() * (100/_zoom));
