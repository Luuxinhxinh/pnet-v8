...
function apiGetRunningWrappers() {
	...
	exec($cmd, $o_qemu, $rc);
	// Running docker nodes via the broker's read-only docker_ps_count verb
	// (Stage 5) — the count is computed root-side; no :4243 dial from www-data.
	$o_docker = [];
	broker_exec('docker_ps_count', [], $o_docker, $rc);
...
function apiSetUksm($p) {
     ...
     broker_exec('wrapper', ['action' => $action], $o, $rc);
     if ($rc == 0 ) {
                $output['code'] = 200;
